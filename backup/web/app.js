...
async function loadStudents(q) {
    ...
    students.forEach((student) => {
        ...
        const photoSrc = student.photoThumb ? `/${student.photoThumb}` : (student.photo ? `/${student.photo}` : '');
        const photoCell = photoSrc ? `<img class="preview" src="${photoSrc}" alt="photo" style="height:40px;width:40px;border-radius:6px;object-fit:cover">` : '-';
        row.innerHTML = `
            <td>${student.id}</td>
            <td>${photoCell}</td>
            <td>${student.name}</td>
...
studentForm.addEventListener('submit', async (event) => {
    ...
    // If a photo file is selected, upload it to the photo endpoint
    if (photoInput && photoInput.files && photoInput.files.length && newId) {
        const fd = new FormData();
        // validate on client: max 2MB and image mime
        const file = photoInput.files[0];
        if (file.size > 2 * 1024 * 1024) {
            alert('Photo too large. Max 2 MB');
            return;
        }
        ...
        fd.append('file', file);
        await fetch(`/api/students/${newId}/photo`, { method: 'POST', body: fd });
    }
...
tableBody.addEventListener('click', async (event) => {
    ...
    if (action === 'edit') {
        ...
        // show existing photo preview when editing
        if (photoPreview) {
            if (student.photo) {
